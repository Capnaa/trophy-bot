...
- **QUIRK — substring matching**: searching "gold" matches "Golden Medal" and any other name containing "gold"; the lowest-ID match is returned with no ambiguity warning. Input that normalizes to empty (e.g. "!!!" or pure emoji) matches EVERY trophy — `"".includes(...)` semantics in `checkName` — so `/show` returns the guild's lowest-ID trophy WITH A NON-EMPTY NORMALIZED NAME instead of "not found" (trophies whose own names normalize to empty are skipped by the `checker &&` guard at `globals.js:141`).
