...
1. **ID branch** (`globals.js:122-129`): if `parseInt(input)` is not NaN (i.e. the input *starts* with digits, optionally signed), the raw input string is interpolated into the quick.db path `data.${guild}.trophies.${input}`. If that path exists, the **raw input string** is returned as the ID.
   ...
   - BUG (path traversal): the raw input is used as a quick.db dot path. Input `1.name` parses as number 1 and `has('...trophies.1.name')` is true if trophy 1 exists — getTrophy returns `"1.name"` as a "valid" trophy ID. Callers then `get('...trophies.1.name')` which returns the *name string* as the trophy object; in `/award` this pushes the bogus ID `"1.name"` into the user's trophy array (award.js:67, which completes first), but the subsequent score `add` (award.js:68) **throws before writing**: `object.value` is undefined, so the increment is `NaN`; `client.db.guilds` is a `db.table` instance, and quick.db 7.1.3's `table.add` has an `isNaN(value)` guard that throws `TypeError` ("Must specify value to add…") **before** arbitrate/the add method ever runs (quick.db src/index.js:269-272), i.e. before any write. The throw is synchronous, so it lands in the dispatcher catch and the user sees the generic error embed; `trophyValue` is never modified, `data.trophiesAwarded` is not incremented, and doRewardRoles never runs. The corruption vector is therefore **array-only**: an orphan/garbage ID in the user's trophies array, not a poisoned score. Production corroborates: 0 NaN/null `trophyValue` among 9,583 user records, 0 dotted IDs among 60,554 awards.
...
- 512 trophies/guild (`>= 512` check, so 512 is the hard cap).
...
- BUG (inherited): the getTrophy path-traversal input (e.g. `1.name`) pushes a garbage ID into the user's trophies array (award.js:67 completes), then the score `add` at award.js:68 throws inside quick.db (`client.db.guilds` is a `db.table`; `table.add`'s `isNaN(value)` guard throws `TypeError` before arbitrate/the add method ever runs — quick.db src/index.js:269-272 — so before any write) into the dispatcher catch — the user gets the generic error embed, `trophyValue` is never modified, `data.trophiesAwarded` is not incremented, and doRewardRoles never runs. Corruption is array-only (an orphan ID entry); the "permanently poisoned score" class of data does not and cannot exist (production: 0 NaN/null `trophyValue` in 9,583 user records, 0 dotted IDs in 60,554 awards).
...
- QUIRK: awarding to a user with no record **creates** it (award.js:57 defaults the missing array with `?? []`; award.js:67-68 write it back and add the score) — like /clear, award is a user-record-creation path.
...
- CLAUDE.md: "Check role rewards and assign if threshold reached" — in the running v14 code this silently does nothing (except in Administrator guilds, where it runs — see the shared doRewardRoles note).
...
- **Score**: always `SUM(trophies.value)` via JOIN over `user_trophies`; never store it. This retires the entire class of trophyValue-desync bugs (revoke pop bug, path-traversal orphan array entries, cleanse mismatches).
