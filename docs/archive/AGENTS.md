...
- Node.js v18 bot resides in `commands/`, `events/`, and `globals.js`; Rust rewrite lives under `src/` with SQLx migrations in `migrations-sql-example/`.
- Legacy data lives in `bot_db.json` and `guilds_db.json`; SQLite snapshots `json.sqlite` assist with migration validation.
- Documentation and research notes: `README_RUST.md`, `MIGRATION.md`, `CLAUDE.md/CLAUDE_OLD.md`, and command specs under `DISCORD_COMMANDS_DOCUMENTATION.md`.

## Migration Objective
- Cut over from quick.db blobs (2 SQLite rows containing everything) to a SeaORM schema that runs on SQLite (dev) and PostgreSQL (prod) so data becomes queryable, reliable, and consistent with the Rust rewrite.
- `MIGRATION.md` highlights the blockers we must fix during the rewrite: `revoke` pops the wrong trophies, `trophyValue` counters desync, bot counters are cumulative-only, 43 trophies lack metadata, and user arrays store duplicate string IDs.
- Schema target (SeaORM entities + migrations): `guilds`, `trophies` (with `legacy_id`), `user_trophies` (duplicates allowed, `awarded_at` timestamps), `guild_settings` (defaults applied), `role_rewards`, `leaderboard_panels`, and `bot_stats`.
- Import pipeline: read quick.db via `LegacyData` (loads `bot`/`guilds` JSON from `json.sqlite`), wrap work in a transaction, insert 2,493 guilds with `imsafe` flag, create 10,853 trophies (fill missing creator/created/signed/details, parse dedication/image), map `(guild, legacy_id) → new_id`, then walk every user array to insert 60,554 award rows (preserving duplicates, warning on orphans).
- Afterwards copy settings with defaults (`dedication_display=2`, etc.), rewards, permissions, panels, and recalc global stats directly from the relational data (ignore `bot_db.json` counters except per-command usage). Document every schema change in SeaORM migrations and mirror decisions in `MIGRATION.md`.
- Validation before commit: assert the expected counts, query for orphaned awards, recompute leaderboard samples (`SUM(t.value)`) to ensure scores align, and plan the 30–60 minute single cutover with backups + Node bot hot for 24h rollback.
...
- `npm install && npm start` launches the legacy Discord.js bot; `node index.js` works for quick runs.
- Rust target: `cargo build` for debug, `cargo build --release` for production-ready binaries, and `sqlx migrate run` to apply DB schema.
- Docker workflow via `./dev.sh [build|run|up|down|logs|clean]` mirrors production stack.

## Coding Style & Naming
- JavaScript uses 2-space indentation, async/await, and avoids unused commons stubs; prefer utility functions in `globals.js`.
- Rust code favors modules with small functions, logging through `log` crate macros (`info!`, `warn!`), and typed SQLx queries; main.rs should only wire modules.
- Assets (trophy images) follow `{guildId}_{trophyId}.{ext}` naming and live in `images/`.

## Testing Guidelines
- Run `npm test` (Jest) for legacy code if re-enabled; prefer targeted script tests when modifying Node commands.
- Rust rewrite must pass `cargo test` plus any doctests; add integration tests for command flows and migration utilities.
- Validate migrations by importing `bot_db.json`/`guilds_db.json` into SQLite dev DB, then comparing counts via SQLx queries.

## Commit & PR Process
- Follow Conventional Commit style (`feat:`, `fix:`, `chore:`) observed in history; explain scope (`feat(commands): add revoke fix`).
- Pull requests should describe changes, include reproduction/testing notes, link issues, and attach screenshots for user-facing embeds.
- Document migration-impacting changes in `MIGRATION.md` and schema changes in SQL migrations.
